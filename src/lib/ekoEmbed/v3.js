...
class EkoEmbedV3 {
    ...
    load(id, options) {
        ...
        this.iframe.setAttribute(
            'src',
            `https://${options.env || ''}embed.eko.com/instance?${stringifyQueryParams(embedParams)}`
        );
    }
    ...
    onEkoEventFired(event) {
        ...
        if (!msg.event || msg.embedid !== this.iframe.id || msg.embedapi !== INSTANCE_EMBED_API) {
            return;
        }
